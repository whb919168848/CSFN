...
def val_loc_one_epoch(args, Val_Loader, model, epoch=0):
    ...
    with torch.no_grad():
        ...
        for step, (img, img_tencrop, label, gt_boxes, path, pre_lable) in enumerate(Val_Loader):
            ...
            output1, map, _ = model(x=img, label=label, phase='test', text=fg_text_features)#
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_box_flag', type=bool, default=True)
    parser.add_argument('--save_img_dir', type=str, default='save_img')

    parser.add_argument('--threshold', type=float, default=[0.1,0.2,0.25,0.3,0.35,0.4,0.45])
    ...
    checkpoint = torch.load('/home/dell/data/code/whb/CSFN/logs/ILSVRC/FINAL/0best_loc.pth.tar')
